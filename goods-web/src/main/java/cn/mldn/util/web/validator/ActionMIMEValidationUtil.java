...
public class ActionMIMEValidationUtil {
	...
	public void validateMime(){	//进行MIME类型的检测
		if(this.multipartResolver.isMultipart(request)){	//表单被封装了，那么就意味着有可能需要上传
			String rule = null;	//规则读取的时候需要考虑没有资源信息的情况
			try{
				rule = messageSource.getMessage(this.key+".mime.rule",null,null);
			}catch(Exception e){
				rule = messageSource.getMessage( "mime.rule",null,null);
			} 
