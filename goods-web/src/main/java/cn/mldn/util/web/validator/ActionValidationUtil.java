...
public class ActionValidationUtil {
	...
	/**
	 * 必须明确的传递要进行验证的Action信息以及具体验证规则处理
	 * ActionObject Action程序类
	 * @param request 请求对象
	 * @param rule 规则
	...
	/**
	 * 根据数据的验证规则，对外部而言，此方法可以不需要知道
	 * 因为所有的错误信息最终都保存在Map集合之中
	 */
	public void handleValidator(){
			...
			switch(temp[1]){
				case "int":  
					if(!ValueRuleValidator.isInt(parameterValue)){	//验证不通过
						//key为参数名称，value为message中定义的提示文字信息
						errors.put(temp[0], this.messageSource.getMessage("validation.int.msg", null,null));
					} 
					break;
				case "long":
					if(!ValueRuleValidator.isLong(parameterValue)){	//验证不通过
						errors.put(temp[0],this.messageSource.getMessage("validation.long.msg",null,null));
					}
					break;
				case "double":
					if(!ValueRuleValidator.isDouble(parameterValue)){//验证不通过
						errors.put(temp[0], this.messageSource.getMessage("validation.double.msg", null,null));
					}
					break;
				case "string":
					if(!ValueRuleValidator.isString(parameterValue)){//验证不通过
						errors.put(temp[0], this.messageSource.getMessage("validation.string.msg", null,null));
					}
				break;
				case "date":
					if(!ValueRuleValidator.isDate(parameterValue)){	//验证不通过
						errors.put(temp[0], this.messageSource.getMessage("validation.date.msg", null,null));
					}
				break;
				case "datetime":
					if(!ValueRuleValidator.isDateTime(parameterValue)){	//验证不通过
						errors.put(temp[0], this.messageSource.getMessage("validation.datetime.msg",null,null));
					}
				break;
				case "rand":
					if(!ValueRuleValidator.isRand(parameterValue, (String)this.request.getSession().getAttribute("rand"))){	//验证没通过
						errors.put(temp[0],this.messageSource.getMessage("validation.rand.msg", null,null));
					}
				break;
				case "boolean":
					if(!ValueRuleValidator.isBoolean(parameterValue)){
						errors.put(temp[0], this.messageSource.getMessage("validation.boolean.msg", null, null));
					}
				break; 
			} 
		} 
